...
void shaderSetup(void)
{
	...
	program = InitShader("vs.shader", "fs.shader");   // hàm InitShader khai báo trong Angel.h
...
void wall_build() {
	...
	instance_room = Translate(4.5, -0.18, -0.357);		//giữa cửa sổ
	changeColor(225, 296, 155);
...
//=============================BÀN HỌC=======================
GLfloat zn, dcngan_keo;  //z-near và độ chuyển: biến kiểu GLfloat lưu trữ giá trị z của không gian hiển thị  (viewport)
						// độ chuyển (khoảng cách mà ngăn kéo đc kéo ra hoặc đẩy vào)
GLfloat theta[] = { 0,0,0,0 };//gồm 4 giá trị GLfloat được sử dụng để lưu trữ các góc xoay, có thể được sử dụng để điều chỉnh các hướng quay của chiếc bàn
mat4 instance_ban;//lưu trữ các thông tin về vị trí và hướng quay của chiếc bàn trong không gian thế giới 3D

void matban(GLfloat w, GLfloat l, GLfloat h)
{
	instance = Scale(w, h, l); //biến đổi tỷ lệ ma trận cục bộ
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, ctm_phong* model * instance_room * instance_ban * instance); 
	//đưa về vị trí, kích thước trong thế giới 3d
	glDrawArrays(GL_TRIANGLES, 0, NumPoints); //vẽ hình chóp chỉ định từ mảng đỉnh theo dạng tam giác
}
...
void ban_hoc() {
	//mặt bàn
	instance_ban = Identity(); // ma trận cục bộ bằng ma trận đơn vị để các giá trị trong ma trận cục bộ không bị ảnh hưởng trước đó
	changeColor(200, 100, 10);
	matban(0.6f, 0.4f, 0.01f);//tỷ lệ
	//chân trái trc
	instance_ban = Translate(-0.29, -0.155, -0.19); // di chuyển đén vị trí trong không gian 3d
	...
	//tu_ban
	instance_ban = Translate(0.225, -0.25, 0);  //di chuyển vị trí của đối tượng trong kgian 3d
	changeColor(250, 200, 35);
	tu_ban(0.15, 0.4, 0.01);    // vẽ phần trên 
	instance_ban = Translate(0.15, -0.18, 0);
	changeColor(250, 200, 35);
	tu_ban(0.01, 0.4, 0.15);  // vẽ phần bên trái
	instance_ban = Translate(0.3, -0.18, 0);
	changeColor(250, 200, 35);
	tu_ban(0.01, 0.4, 0.15); // vẽ phần bên phải
	instance_ban = Translate(0.3, -0.18, -0.195) * RotateY(theta[1]) * Translate(-0.075, 0, 0);
	//Di chuyển vị trí của đối tượng theo các tọa độ và quay nó theo góc theta[1] quanh trục Y
	// rotate y ko quay do theta =0, chỉ mang tính chất giúp dễ dàng thay đổi góc quay nếu muốn mà ko cần sửa các chỗ khác
	changeColor(250, 200, 35);
	tu_ban(0.15, 0.01, 0.15);  //phần dưới của tủ bàn
	//máy tính
	instance_ban = Translate(-0.25-dcmay_tinh, 0, 0);
	may_tinh();
}

GLfloat heso = -0.8; //khoảng cách giữa cách máy tính và số lượng đặt đc trong phòng
GLfloat dc = -4.8; // độ dịch chuyển 
//Điểm mốc ban đầu của bàn học là (3.5, -1.6, dc) điểm mốc tiếp theo dựa vào i
void dayban1() {
	for (int i = 0; i < 11; i++) {
...
void rem_build() {
	...
	changeColor(33, 229, 236);
	rem(0.5, chieucao_rem, 0.002);
}
// ==============MÁY CHIẾU====================
mat4 instance_maychieu;
float chieu_dai = 1;
float cuon_may, quay_maychieu;
...
void maychieu_build() {
	...
	instance_maychieu = instance_maychieu * Translate(0, -0.15, 0) * RotateY(quay_maychieu);
...
void room_build() {
	...
	veGheGV();
	instance_room = Translate(-4.7, 0.5, -3.5) * RotateY(90) * Scale(8.5, 3.5, 3);
	rem_build();
	instance_room = Translate(-4.7, 0.5, 2) * RotateY(90) * Scale(8.5, 3.5, 3);
	rem_build();
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'K':
		dcngan_keo = 0;
		//if (dcngan_keo <= 0) dcngan_keo -= (dcngan_keo - 0.1);
		break;
		//bật tắt màn hình
	case 's':
		man = 1;
	...
	case 'e':
		quay_maychieu += 0.5;
		if (quay_maychieu >= 25) quay_maychieu -= (quay_maychieu - 25);
		break;
	case 'E':
		quay_maychieu += -0.5;
		if (quay_maychieu <= -25) quay_maychieu -= (quay_maychieu + 25);
